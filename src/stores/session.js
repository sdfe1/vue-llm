...
export const useSessionStore = defineStore(
  "session",
  () => {
    const session = ref({ [DEFAULT_SESSION_NAME]: [] });//存储所有会话的消息内容
    const curname = ref(DEFAULT_SESSION_NAME);//当前正在查看/编辑的会话名称
    const time = ref({});//时间戳
    const reason = ref({ [DEFAULT_SESSION_NAME]: [] });//存储每个会话中每条消息的推理内容
    const showreason = ref({ [DEFAULT_SESSION_NAME]: [] });//控制每个会话中每条消息的推理内容是否显示
    const visibility = ref({ [DEFAULT_SESSION_NAME]: false });//标记会话是否在侧边栏可见
    const pendingConversation = ref(DEFAULT_SESSION_NAME);//记录当前代输入的空白会话名称
    const model = ref(MODEL_OPTIONS[0].value);//当前选择的模型类型
    ...
    const ensureConversation = (
      ...
      const messages = session.value[name];
    ...
    const getcurmsgs = () => {
      ensureConversation();
      return session.value[curname.value];
    };
